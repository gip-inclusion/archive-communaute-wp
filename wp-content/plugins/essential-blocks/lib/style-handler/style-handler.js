...
//Generate Styles 
(() => {
  const { editor_type } = eb_style_handler;
  ...
  const regexForBlockNameTest = /^essential\-blocks\//g;
    ...
    const cssObjectMaker = (blocks) => {
      for (const item of blocks) {
        ...
        if (isFunction(isReusableBlock) && isReusableBlock(item)) {
          ...
          const parsedBlocks = !isEmpty(reusableBlock) ? parse(
          ...
          if (parsedBlocks) {
            for (const blockItem of parsedBlocks) {
              ...
              if (blockMeta && blockRoot === "essential_block") {
          ...
          cssObjectMaker(innerBlocks);
        } else if (blockMeta && blockRoot === "essential_block") {
          styles[blockId] = blockMeta;
        }
        if (innerBlocks.length > 0) {
          cssObjectMaker(innerBlocks);
