...
class BP_Member_Type_User_Page {
	...
	public function user_member_type_bulk_dropdown() {
		...
		$query = "SELECT DISTINCT ID, post_title FROM {$wpdb->posts} WHERE post_type = %s AND post_status = %s";
		...
		<?php foreach ( $active_member_type as $member_type ) : ?>
			<?php
			$member_type_name 	= bmt_get_member_type_key( $member_type->ID );
			?>
			<option value="<?php echo $member_type_name; ?>"><?php echo $member_type->post_title; ?></option>
		<?php endforeach; ?>
		</select>
