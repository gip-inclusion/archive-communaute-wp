...
function bmt_member_post_by_type($member_type) {
    ...
    return apply_filters( 'bmt_member_post_by_type', $post_id );
}
...
function bmt_get_removed_member_types(){
    ...
    if ($bmt_query->have_posts()):
        while ($bmt_query->have_posts()):
            ...
            $post_id = get_the_ID();
            $name = bmt_get_member_type_key( $post_id );
            $bmt_ids[] = array(
                'ID' => $post_id,
                'name' => $name,
            );
 ...
 * @return mixed|string
 */
function  bmt_get_member_type_key( $post_id ) {
	...
	return apply_filters( 'bmt_get_member_type_key', $key );
}