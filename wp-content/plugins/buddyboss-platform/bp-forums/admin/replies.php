...
if ( ! class_exists( 'BBP_Replies_Admin' ) ) :
	...
	class BBP_Replies_Admin {
		 ...
		 *                        the columns
		 * @return array $columns Forums reply columns
		 */
		...
		public function row_actions( $actions, $reply ) {
			...
			if ( current_user_can( 'moderate', $reply->ID ) ) {
				if ( in_array( $reply->post_status, array( bbp_get_public_status_id(), bbp_get_spam_status_id() ) ) ) {
					$spam_uri = wp_nonce_url(
						add_query_arg(
							array(
								'reply_id' => $reply->ID,
								'action'   => 'bbp_toggle_reply_spam',
							),
							remove_query_arg( array( 'bbp_reply_toggle_notice', 'reply_id', 'failed', 'super' ) )
						),
						'spam-reply_' . $reply->ID
					);
					if ( bbp_is_reply_spam( $reply->ID ) ) {
						$actions['spam'] = '<a href="' . esc_url( $spam_uri ) . '" title="' . esc_attr__( 'Mark the reply as not spam', 'buddyboss' ) . '">' . esc_html__( 'Not spam', 'buddyboss' ) . '</a>';
					} else {
						$actions['spam'] = '<a href="' . esc_url( $spam_uri ) . '" title="' . esc_attr__( 'Mark this reply as spam', 'buddyboss' ) . '">' . esc_html__( 'Spam', 'buddyboss' ) . '</a>';
					}
				}
			}
			...
			if ( current_user_can( 'delete_reply', $reply->ID ) ) {
				if ( bbp_get_trash_status_id() === $reply->post_status ) {
					...
					$actions['untrash'] = "<a title='" . esc_attr__( 'Restore this item from the Trash', 'buddyboss' ) . "' href='" . esc_url( add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => bbp_get_reply_post_type() ), admin_url( 'edit.php' ) ) ), wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $reply->ID ) ), 'untrash-post_' . $reply->ID ) ) ) . "'>" . esc_html__( 'Restore', 'buddyboss' ) . '</a>';
				} elseif ( EMPTY_TRASH_DAYS ) {
					$actions['trash'] = "<a class='submitdelete' title='" . esc_attr__( 'Move this item to the Trash', 'buddyboss' ) . "' href='" . esc_url( add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => bbp_get_reply_post_type() ), admin_url( 'edit.php' ) ) ), get_delete_post_link( $reply->ID ) ) ) . "'>" . esc_html__( 'Trash', 'buddyboss' ) . '</a>';
				}

				if ( bbp_get_trash_status_id() === $reply->post_status || ! EMPTY_TRASH_DAYS ) {
					$actions['delete'] = "<a class='submitdelete' title='" . esc_attr__( 'Delete this item permanently', 'buddyboss' ) . "' href='" . esc_url( add_query_arg( array( '_wp_http_referer' => add_query_arg( array( 'post_type' => bbp_get_reply_post_type() ), admin_url( 'edit.php' ) ) ), get_delete_post_link( $reply->ID, '', true ) ) ) . "'>" . esc_html__( 'Delete Permanently', 'buddyboss' ) . '</a>';
